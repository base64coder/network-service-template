...
public class JdbcRepository<T, ID> implements BaseRepository<T, ID> {
    ...
    private final com.dtc.core.persistence.dialect.Dialect dialect;
    ...
    public JdbcRepository(DataSourceProvider dataSourceProvider) {
        ...
        try (Connection conn = dataSourceProvider.getConnection()) {
            this.dialect = com.dtc.core.persistence.dialect.DialectFactory.getDialect(conn);
     ...
     * 判断主键是否为自动生成
     */
    private boolean isAutoGeneratedKey() {
        if (primaryKeyField == null) {
            return false;
        }
        
        Id idAnnotation = primaryKeyField.getAnnotation(Id.class);
        return idAnnotation != null && idAnnotation.keyType() == Id.KeyType.AUTO;
    }
            ...
            if (isAutoGeneratedKey()) {
                try (ResultSet generatedKeys = stmt.getGeneratedKeys()) {
                    if (generatedKeys.next()) {
                        ...
                        setPrimaryKeyValue(entity, convertValue(key, primaryKeyField.getType()));
                    } else {
                        // 某些数据库可能需要特定的 SQL 查询
                        String identitySql = dialect.getIdentitySelectString(metadata.getTableName());
                        if (identitySql != null) {
                            try (Statement idStmt = conn.createStatement();
                                 ResultSet idRs = idStmt.executeQuery(identitySql)) {
                                if (idRs.next()) {
                                    Object key = idRs.getObject(1);
                                    setPrimaryKeyValue(entity, convertValue(key, primaryKeyField.getType()));
                                }
                            }
                        }
